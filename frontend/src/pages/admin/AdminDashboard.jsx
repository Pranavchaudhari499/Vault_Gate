...
import { Shield, Activity, Users, AlertTriangle, TrendingUp, Globe, Lock } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        fetchMetrics();
        fetchGatewayMetrics();
        // Refresh gateway metrics every 5 seconds for real-time monitoring
    ...
    const fetchGatewayMetrics = async () => {
        try {
            const response = await axios.get('/api/admin/gateway-metrics');
            setGatewayMetrics(response.data);
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Gateway Rate Limiting Monitor */}
            <div className="bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/30 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                        <Lock className="w-6 h-6 text-red-400" />
                        <div>
                            <h2 className="text-xl font-semibold text-white">Gateway Rate Limiting (Live)</h2>
                            <p className="text-sm text-gray-400">Production-grade Redis-backed enforcement</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-slate-900/50 border border-slate-700 rounded-lg p-4">
                        ...
                        <div className="text-2xl font-bold text-white">{gatewayMetrics.totalRequests.toLocaleString()}</div>
                    </div>
                    <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                        <div className="text-sm text-gray-400 mb-1">Allowed</div>
                        <div className="text-2xl font-bold text-green-400">{gatewayMetrics.allowedRequests.toLocaleString()}</div>
                    </div>
                    <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                        <div className="text-sm text-gray-400 mb-1">Blocked</div>
                        <div className="text-2xl font-bold text-red-400">{gatewayMetrics.blockedRequests.toLocaleString()}</div>
                    </div>
                    <div className="bg-orange-500/10 border border-orange-500/30 rounded-lg p-4">
                        <div className="text-sm text-gray-400 mb-1">Active Offenders</div>
                        <div className="text-2xl font-bold text-orange-400">{gatewayMetrics.activeOffenders}</div>
                    </div>
                </div>

                <div className="mt-4 flex items-center justify-between p-3 bg-slate-900/50 border border-slate-700 rounded-lg">
                    <div className="flex items-center space-x-3">
                        <Shield className="w-5 h-5 text-blue-400" />
                        <div>
                            <div className="text-sm font-medium text-white">Block Rate</div>
                            <div className="text-xs text-gray-400">Percentage of requests blocked by gateway</div>
                        </div>
                    </div>
                    <div className="text-2xl font-bold text-blue-400">{gatewayMetrics.blockRate}%</div>
                </div>
                ...
                <div className="mt-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                    <div className="flex items-start space-x-2">
                        ...
                        <div className="text-xs text-gray-300">
                            <span className="font-semibold text-blue-400">Production Enforcement:</span> 100 req/min (authenticated), 50 req/min (anonymous). 
                            Violators are temporarily blocked for 5-10 minutes. All metrics update in real-time via Redis.
                        </div>
                    </div>
                </div>
            </div>
